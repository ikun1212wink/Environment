...
uint8_t CAN_SendUint8(uint32_t std, uint8_t *data,char canNum,uint32_t DateLength)
{
	...
	if(canNum == 1)i = HAL_CAN_AddTxMessage(&hcan1,&TxMes.header,TxMes.data,  &CAN_Tx_Mailbox);
	if(canNum == 2)i = HAL_CAN_AddTxMessage(&hcan2,&TxMes.header,TxMes.data,  &CAN_Tx_Mailbox);
	return i;
}
