...
/* Private macro -------------------------------------------------------------*/
// 串口2数据帧长度
#define USART2_RX_DATA_FRAME_LEN    (18u)    
// 串口2接收缓冲区长度
#define USART2_RX_BUF_LEN           (USART2_RX_DATA_FRAME_LEN + 6u)    
// 串口1接收缓冲区长度
...
void USART_AddMsg(drv_uart_t *drv, uint8_t *data, uint8_t data_cnt)
{
    ...
    if((drv->tx_data_cnt + data_cnt) <= drv->tx_buf_len) {
        memcpy((uint8_t*)(drv->tx_buf + drv->tx_data_cnt), (uint8_t*)data, data_cnt);
        drv->tx_data_cnt += data_cnt;
    }
    else {
        // @Error: out of range
...
void USART_StartTx(drv_uart_t *drv)
{
    ...
    {
        case DRV_UART1:
            drv->errno = (drv_errno_t)HAL_UART_Transmit(&huart1, drv->tx_buf, drv->tx_data_cnt, 1000);
            break;
        
        case DRV_UART2:
            drv->errno = (drv_errno_t)HAL_UART_Transmit(&huart2, drv->tx_buf, drv->tx_data_cnt, 1000);
            break;
        
//        case DRV_UART3:
//            drv->errno = (drv_errno_t)HAL_UART_Transmit(&huart3, drv->tx_buf, drv->tx_data_cnt, 1000);
//            break;
        
        case DRV_UART4:
            drv->errno = (drv_errno_t)HAL_UART_Transmit(&huart4, drv->tx_buf, drv->tx_data_cnt, 1000);
            break;
        
        case DRV_UART5:
            drv->errno = (drv_errno_t)HAL_UART_Transmit(&huart5, drv->tx_buf, drv->tx_data_cnt, 1000);
            break;
...
void USART_StartTxDma(drv_uart_t *drv)
{
    ...
    {
        case DRV_UART1:
            drv->errno = (drv_errno_t)HAL_UART_Transmit_DMA(&huart1, drv->tx_buf, drv->tx_data_cnt);
            break;
        
        case DRV_UART2:
            drv->errno = (drv_errno_t)HAL_UART_Transmit_DMA(&huart2, drv->tx_buf, drv->tx_data_cnt);
            break;
        
//        case DRV_UART3:
//            drv->errno = (drv_errno_t)HAL_UART_Transmit_DMA(&huart3, drv->tx_buf, drv->tx_data_cnt);
//            break;
        
        case DRV_UART4:
            drv->errno = (drv_errno_t)HAL_UART_Transmit_DMA(&huart4, drv->tx_buf, drv->tx_data_cnt);
            break;
        
        case DRV_UART5:
            drv->errno = (drv_errno_t)HAL_UART_Transmit_DMA(&huart5, drv->tx_buf, drv->tx_data_cnt);
            break;
